...
export const tempMongooseStore = defineStore("mongooseTemp", () => {
  ...
  function init(dbName: string, collectionName) {
    ...
    if (mongoose.conn == "") {
      mongoose.conn = mongoose.mongoose.createConnection(url, {
        dbName: dbName
      });
    } else if (
      dbName == mongoose.currentDB &&
      collectionName == mongoose.currentCollection
    ) {
      return;
    ...
    mongoose.schema = new Schema(mongoose.types, {
