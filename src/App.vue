...
const startMongo = (dbName: string, collectionName: string) => {
  const url: string = "mongodb+srv://cherish:Yuchenyu1@cluster0.pa95kl8.mongodb.net/?retryWrites=true&w=majority";
...
ipcRenderer.invoke("config-get").then((config: Config) => {
...
function modelInit() {
  ...
  const defaultModel = startMongo(configStore.config.repository.name, configStore.config.repository.classes[0]);
// 共享状态
