...
import { ref } from "vue";
import { WordMap } from "@/interface/typings";
...
export const wordsStore = defineStore("wordsStore", () =>
{
  ...
  function getWordList(name:string): Word[] {
    let item = localStorage.getItem("words");
    if (item) {
  ...
  return { wordsList, collectionList, setCollectionList, setWoldList, getWordList, toLearnWord};
}, {
  persist: {
    enabled: true,
    strategies: [
      {storage: localStorage, key: "collection"}
    ]
  }
})
