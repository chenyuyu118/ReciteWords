<template>
  <div class="flex-container">
    <div>
      ...
      <el-input
        v-model="configStore.config.repository.name"
        :disabled="true"
    ...
    <div class="user-repo-box">
      <el-form v-show="userConfig?.length === 0 || addUserConfigFlag">
        ...
        <el-form-item label="单词分类">
          <div v-show="isNewClass" style="margin-bottom: 8px">
            <el-input
              ref="inputClass"
              v-model="newClass"
              style="width: 280px"
            ...
            ></el-input>
            <el-button style="margin-left: 10px" @click="addClass"
            >添加
            </el-button
            >
          </div>
          ...
          <el-checkbox-group v-model="newConf.classes">
            <el-checkbox-button label="absorb"> 已掌握</el-checkbox-button>
            <el-checkbox-button label="non_absorb"> 未掌握</el-checkbox-button>
            <span v-for="item in newConf.classes">
              <span v-if="item !== 'absorb' && item !== 'non_absorb'">
                <el-checkbox-button :label="item"> </el-checkbox-button>
              </span>
            </span>
          </el-checkbox-group>
          <el-button
            ...
            @click="isNewClass = true"
          />
        </el-form-item>
        <div style="text-align: center; margin-top: 10px">
          <el-button type="primary" @click="addUserConfig">提交</el-button>
          <el-button
            v-if="userConfig?.length !== 0"
            type="primary"
            @click="addUserConfigFlag = false"
          >取消
          </el-button
          >
        </div>
      ...
      <div class="scroll-box">
        <el-scrollbar>
          <el-collapse
            v-for="(item, index) in userConfig"
            v-if="userConfig?.length !== 0"
            v-show="!addUserConfigFlag"
          >
            <el-collapse-item :title="item.name" :name="index">
              <span v-for="i in item.classes">
                <el-tag style="margin-right: 6px">{{
                    i === "non_absorb" ? "未掌握" : i === "absorb" ? "掌握" : i
                  }}</el-tag>
              </span>
            </el-collapse-item>
          </el-collapse>
    ...
    <div class="button-flex-container">
      <div>
        <el-button
          v-show="userConfig?.length !== 0 && !addUserConfigFlag"
          type="primary"
          @click="saveConfig"
        >保存
        </el-button>
...
import type { UserConfig } from "@/interface/typings";
...
const userConfig = computed({
  get() {
    ...
    if (userConfig1.value == undefined) userConfig1.value = [];
...
function addUserConfig() {
  ...
  if (userConfig1.value != null) userConfig.value = userConfig1.value;
  newConf.value = {
    name: "",
    password: "",
    classes: [],
